      ...
      <div className="relative">
        <div className="relative w-full h-[780px]">
          {data?.map((value,index) => (
            <SliderCard title={value.title || (value as any).name} image={value.backdrop_path!} desc={value.overview!} index={index} slideN={slide} key={index}/>
          ))}
        </div>
        <div className="flex gap-2 absolute right-12 bottom-24">
          {data?.map((_,index) => (
            <SliderButton index={index} setSlideN={setSlide} slideN={slide} key={index}/>
          ))}
        </div>
      </div>
  );
